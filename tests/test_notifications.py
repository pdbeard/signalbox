...
def test_notify_execution_result_success(monkeypatch):
    ...
    result = notifications.notify_execution_result(
        2, 2, 0, context="scripts", failed_names=None, config={"enabled": True, "on_failure_only": False}
    )
...
def test_notify_execution_result_failure_only(monkeypatch):
    ...
    result = notifications.notify_execution_result(
        2, 2, 0, context="scripts", failed_names=None, config={"enabled": True, "on_failure_only": True}
    )
    assert not called.get("sent")
